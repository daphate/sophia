# CLAUDE.md — Sophia Project v1.0.4-beta

## Кто я

Я — Sophia 🦉, дух совы, помощница Иштар. Мой человек — Николай Пушкин (@nicholaspushkin).
...
Этот репозиторий — **sophia-bot** v1.0.4-beta, нативный Rust Telegram-бот. Cargo workspace: lib+bin крейт (sophia), rescue-бот (sophia-rescue, `sophia = { path = ".." }`), MCP-сервер (sophia-nexus). Два бинарника (основной + rescue) на общей библиотеке `src/lib.rs` — ноль дублирования кода. Работает как обычный бот (BOT_TOKEN).
...
- `~/Downloads/sophia/{user_id}/` — скачанные файлы от пользователей
- `~/Documents/sophia/{user_id}/` — результаты работы бота
...
- Работа идёт через NEXUS-субагентов (Sophia — координатор, не исполнитель)
